...
def get_last_record(table_id: str, headers: Dict, page_token: str = None) -> Dict:
    record_url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{config.APP_TOKEN}/tables/{table_id}/records"
    if page_token:
        record_url += f"?page_token={page_token}"
    result = requests.get(record_url, headers=headers)
    data = json.loads(result.text)["data"]
    if data["has_more"]:
        # 如果有更多数据，则递归调用
        return get_last_record(table_id, headers, data["page_token"])
    else:
